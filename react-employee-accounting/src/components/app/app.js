...
class App extends Component {
    ...
    // По клику на корзинку удаляется сотрудник. Чтобы его удалить, нужно удалить его из данных. Тут есть вопросы.
    // тк иконка находится в определенном компаненте не связанным с данными которые лежат выше по иерархии.
    // Чтобы их связать, нужно написать свои собственные события. При клике на корзинку и удалении, часть данныхудалится
    // из data, реакт увидит это изменение и перерисует приложение. В emploeersList передаем функцию 
    // onDelete={() => onDelete(id)} Далее опускаемся вниз и в emplListItem на обрботчик события вешаем 
    // эту функци onClick={onDelete} Мы таким действием связываем несколько компонентов которые находятся на 
    // разных уровнях.
    // АЛГОРИТМ удаления элемента из массива с объектами где мы ориентируемся по уникальному идентификатору id:
    //         Сначала мы этот метод передали ниже, и теперь он вызывается с уникальным id. Теперь по этому id 
    //         находим элемент внутри массива чтобы потом его удалить. Ну а после удаления будем изменять стейт
    // для того, чтобы создать новое состояние.
    deleteItem = (id) => {
        this.setState(({ data }) => {
            // один из приемов - чтобы найти определенный объект нам нужен его индекс.
            //             findIndex принимает в себя колбэк функцию которая запускается, если эта функция вернет true
            // то из этого метода будет возвращен номер этого элемента на котором сработала эта функция
            // const index = data.findIndex(elem => elem.id === id) В этой функции мы сравниваем id каждого элемента
            // с уникальным идентификатором который приходит из параметра deleteItem = (id) Если мы найдет такое совпадение, то 
            // у нас будет индекс елемента который нужно удалить
            // elem - каждый элемент по порядку.
            //             Стейт у нас неизменяемый, те мы не можем напрямую менять data.Поэтому создаем новый массив.
            // первый вариант решения с методом slice которй берет часть массива и создает новый:
            // const before = data.slice(0, index);
            // здесь копируем лишь часть массива, сначала до того элемента который нужен, потом - после.
            // const after = data.slice(index + 1); Адлее создаем один большой массив:
            // const newArr = [...before,...after]
            // второй вариант через filter - он тоже создает новый массив:
            return {
